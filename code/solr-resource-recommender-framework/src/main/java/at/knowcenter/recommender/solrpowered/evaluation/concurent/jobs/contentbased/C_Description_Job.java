...
public class C_Description_Job extends C_Strategy_Job implements Callable<List<MetricsExporter>>{

	private List<String> users;

	public C_Description_Job(List<String> users) {
		this.users = users;
	}
	...
	public List<MetricsExporter> evaluate() {
		...
		List<MetricsExporter> metricsExporters = initMetricCalcs("7_" + StrategyType.CN_WeightDescription.name());
		
		System.out.println("Evaluation over " + users.size() + " users");
		
		evaluate_C_Weighted(cf, metricsExporters, StrategyType.CN_WeightDescription);
