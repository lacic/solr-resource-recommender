 ...
 *
 */
public class ReviewBasedRecPS2 implements RecommendStrategy {
	...
	public RecommendResponse recommend(RecommendQuery query, Integer maxReuslts){
		...
		try {
			...
			Map<String, Set<String>> userSellerMap = fetchUserItemMapping(otherUsers, query.getUser());
			...
			for (String commonUser : userSellerMap.keySet()) {
				...
				sellers.retainAll(query.getProductIds());
	...
	private Map<String, Set<String>> fetchUserItemMapping(Set<String> otherUsers, String currentUser) {
		...
		solrParams.set("rows", Integer.MAX_VALUE);
		solrParams.set("fl", "id,users_rated_5,users_rated_4,users_rated_3,users_rated_2,users_rated_1");
		...
		try {
			...
			for (Resource res : resources) {
				...
				if (usersRated1 != null) {
					fillSellersForUsers(userSellersMapping, itemID, usersRated1, currentUser, otherUsers);
				}
				if (usersRated2 != null) {
					fillSellersForUsers(userSellersMapping, itemID, usersRated2, currentUser, otherUsers);
				}
				if (usersRated3 != null) {
					fillSellersForUsers(userSellersMapping, itemID, usersRated3, currentUser, otherUsers);
				}
				if (usersRated4 != null) {
					fillSellersForUsers(userSellersMapping, itemID, usersRated4, currentUser, otherUsers);
				}
				if (usersRated5 != null) {
					fillSellersForUsers(userSellersMapping, itemID, usersRated5, currentUser, otherUsers);
				}
			}
		} catch (SolrServerException e) {
			System.out.println(solrParams);
	...
	private void fillSellersForUsers(Map<String, Set<String>> userSellersMapping, String itemID, 
		...
		for (String purchasingUser :usersThatPurchased) {
			if ((! purchasingUser.equals(currentUser)) && otherUsers.contains(purchasingUser)) {
				Set<String> items = userSellersMapping.get(purchasingUser);
				...
				items.add(itemID);
				userSellersMapping.put(purchasingUser, items);
			}
		}
	}
