...
public class RegionCoocurredPrefAttBasedRec implements RecommendStrategy{
	...
	public RecommendResponse recommend(RecommendQuery query, Integer maxReuslts) {
		...
		try {
			...
			solrParams.set("sort", "region_cooccurred_neighborhood_count desc");
			...
			Thread t = new Thread() {
				@Override public void run() {
					UserSimilarityTracker.getInstance().writeToFile("loc_network_prefatt", user, commonNeighborMap);
				}
			};
			t.start();
