...
public class InterestNeighborOverlapBasedRec implements RecommendStrategy {
	...
	public RecommendResponse recommend(RecommendQuery query, Integer maxReuslts){
		...
		try {
			...
			for (Customer customer : resLists) {				
				...
				List<String> sumGroups = new ArrayList<String>(contentFilter.getCustomer().getInterests());
				sumGroups.addAll(customer.getInterests());
				
				customerScoringMap.put(customer.getId(), intersectedInterests.size() / (double)sumGroups.size());
			}
	...
	public StrategyType getStrategyType() {
		return StrategyType.CF_Soc_Interests_Overlap;
	}
