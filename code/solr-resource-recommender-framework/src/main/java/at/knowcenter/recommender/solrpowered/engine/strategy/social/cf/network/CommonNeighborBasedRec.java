 ...
 *
 */
public class CommonNeighborBasedRec implements RecommendStrategy {
	...
	public RecommendResponse recommend(RecommendQuery query, Integer maxReuslts){
		...
		try {
			...
			Set<String> currentUserNeighbors = new HashSet<String>();
			...
			solrParams = getComparingParams(currentUserNeighbors);
			...
			Map<String, Set<String>> otherUsersNeighbors = new HashMap<String, Set<String>>();
			...
			for (SocialAction socialUser : neighborActions) {
				...
				Set<String> neighbors = otherUsersNeighbors.get(user);
			...
			final Map<String, Integer> commonNeighborMap = new HashMap<String, Integer>();
			for (String user : otherUsersNeighbors.keySet()) {
				Set<String> neighbors = new HashSet<String>(otherUsersNeighbors.get(user));
				neighbors.retainAll(currentUserNeighbors);
				
				commonNeighborMap.put(user, neighbors.size());
			}
			...
			Thread t = new Thread() {
				@Override public void run() {
					UserSimilarityTracker.getInstance().writeToFileInt("soc_network_cn", user, commonNeighborMap);
				}
			};
			t.start();
	...
	private void fillFurtherNeighbors(Map<String, Set<String>> usersNeighbors, List<String> users, String neighbor) {
		for (String user : users) {
			Set<String> neighbors = usersNeighbors.get(user);
			if (neighbors == null) {
				neighbors = new HashSet<String>();
			}
	...
	public StrategyType getStrategyType() {
		return StrategyType.CF_Soc_Network_CN;
	}
