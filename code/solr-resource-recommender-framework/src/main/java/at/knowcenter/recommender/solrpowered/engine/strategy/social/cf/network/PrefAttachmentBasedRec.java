 ...
 *
 */
public class PrefAttachmentBasedRec implements RecommendStrategy {
	...
	public RecommendResponse recommend(RecommendQuery query, Integer maxReuslts){
		...
		try {
			...
			for (String user : otherUsersNeighbors.keySet()) {
				commonNeighborMap.put(user, otherUsersNeighbors.get(user).size() * currentUserNeighbors.size());
			}
			...
			Thread t = new Thread() {
				@Override public void run() {
					UserSimilarityTracker.getInstance().writeToFileInt("soc_network_prefatt", user, commonNeighborMap);
				}
			};
			t.start();
	...
	public StrategyType getStrategyType() {
		return StrategyType.CF_Soc_Network_PrefAttachment;
	}
