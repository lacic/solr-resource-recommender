 ...
 *
 */
public class AdamicAdarBasedRec implements RecommendStrategy {
	...
	public RecommendResponse recommend(RecommendQuery query, Integer maxReuslts){
		...
		try {
			...
			for (String user : otherUsersNeighbors.keySet()) {
				...
				Double aaSum = 0.0;
				for (String intersectingUser : intersectionNeighbors) {
					aaSum += 1.0 / Math.log(otherUsersNeighbors.get(intersectingUser).size());
				}
				
				commonNeighborMap.put(user, aaSum);
			}
	...
	public StrategyType getStrategyType() {
		return StrategyType.CF_Soc_Network_AdamicAdar;
	}
