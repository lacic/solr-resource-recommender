...
public class LocationAdarBasedRec implements RecommendStrategy{
	...
	public RecommendResponse recommend(RecommendQuery query, Integer maxReuslts) {
		...
		try {
			...
			List<String> locationNeighbors = positions.get(0).getLocationNeighbors();
			...
			StringBuilder sb = new StringBuilder("global_neighborhood:(");
			...
			solrParams.set("fl", "id,global_neighborhood");
			...
			solrParams.set("q", sb.toString());
			solrParams.set("rows", Integer.MAX_VALUE);
			solrParams.set("fl", "id,global_neighborhood_count");
			solrParams.set("fq", "-id:" + user);
			...
			for (PositionNetwork neighbourPosition : neighbourPositions) {
				degreeMap.put(neighbourPosition.getUserId(), neighbourPosition.getLocationNeighborsCount());
			}
			
			
			final Map<String, Double> commonNeighborMap = new HashMap<String, Double>();
			for (PositionNetwork otherPosition : otherPositions) {
				List<String> commonNeighbors = otherPosition.getLocationNeighbors();
	...
	public StrategyType getStrategyType() {
		return StrategyType.CF_Location_Network_All_Adar;
	}
