...
public class CF_Social_Job extends RecommenderEvaluator implements Callable<List<MetricsExporter>>{
	...
	public List<MetricsExporter> evaluate() {
		...
		for (int n = 1; n <= resultSize; n++) {
			...
			mCalc.setAlgorithmName("9_CF_Social");
...
//		System.out.println("Evaluation over " + users.size() + " users");
	...
	protected void evaluate_CF_Social(List<String> users, ContentFilter cf, List<MetricsExporter> metricsCalcs) {
		...
		for (String userID : users) {
			...
			List<String> recommendations = getRecommendations(userID, null, resultSize, cf, recommendStrategies.get(StrategyType.CF_Social));
			...
			System.out.println("CF_Social_+" + jobDescription +": Evaluation progress: " + ((currentEvaluatingUser) / (double)userSize) * 100 + " % done");
		}
	}
