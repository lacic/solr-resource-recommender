 ...
 *
 */
public class SellerLocationJaccardBasedRec implements RecommendStrategy {
	...
	public RecommendResponse recommend(RecommendQuery query, Integer maxReuslts){
		...
		try {
			...
			Set<String> sellingRegionsPurchasedFrom = new HashSet<String>();

			for (Resource res : purchasedResources) {
				sellingRegionsPurchasedFrom.add(res.getSellingRegion());
			}
			
			Set<String> otherUsers = fetchUsersFromSellingRegions(sellingRegionsPurchasedFrom, query.getUser());
			...
			for (String commonUser : userSellerMap.keySet()) {
				...
				intersection.retainAll(sellingRegionsPurchasedFrom);
				
				Set<String> union = new HashSet<String>(sellers);
				union.addAll(sellingRegionsPurchasedFrom);
	...
	private Set<String> fetchUsersFromSellingRegions(Set<String> sellingRegionsPurchasedFrom, String currentUser) {
		Set<String> usersFromSellers = new HashSet<String>();
		ModifiableSolrParams solrParams = new ModifiableSolrParams();
		
		StringBuilder sellerBuilder = new StringBuilder("selling_region:(");

		for (String seller : sellingRegionsPurchasedFrom) {
			sellerBuilder.append("\"" + seller + "\" OR ");
		}
		
		if (sellingRegionsPurchasedFrom.size() > 0) {
		...
		solrParams.set("fq", "selling_region:[* TO *]");
	...
	private List<Resource> findPurchasedResources(List<String> productIds) throws SolrServerException {
		ModifiableSolrParams solrParams = createParamsToFindSellingRegions(productIds);
	...
	private Map<String, Set<String>> fetchUserSellerMapping(Set<String> otherUsers, String currentUser) {
		...
		solrParams.set("rows", Integer.MAX_VALUE);
		solrParams.set("fq", "seller:[* TO *] AND selling_region:[* TO *]");
		solrParams.set("fl", "selling_region,users_rated_5,users_rated_4,users_rated_3,users_rated_2,users_rated_1");
		...
		try {
			...
			for (Resource res : resources) {
				String seller = res.getSellingRegion();
	...
	private ModifiableSolrParams createParamsToFindSellingRegions(List<String> purchasedProducts) {
		ModifiableSolrParams solrParams = new ModifiableSolrParams();
		String queryString = 
		...
		solrParams.set("q", queryString);
		solrParams.set("fq", "selling_region:[* TO *]");
		solrParams.set("fl", "selling_region");
