...
public class PicksTotalBasedRec implements RecommendStrategy{
	...
	public RecommendResponse recommend(RecommendQuery query, Integer maxReuslts) {
		...
		try {
			...
			for (Customer commonCustomer : customers) {
				List<String> union = commonCustomer.getFavoriteRegions();
				union.addAll(favoriteRegions);
				
				commonNeighborMap.put(commonCustomer.getId(), (double)union.size());
			}
			...
			Thread t = new Thread() {
				@Override public void run() {
					UserSimilarityTracker.getInstance().writeToFile("loc_content_picks_tot", user, commonNeighborMap);
				}
			};
			t.start();
	...
	public StrategyType getStrategyType() {
		return StrategyType.CF_Loc_Picks_Total;
	}
