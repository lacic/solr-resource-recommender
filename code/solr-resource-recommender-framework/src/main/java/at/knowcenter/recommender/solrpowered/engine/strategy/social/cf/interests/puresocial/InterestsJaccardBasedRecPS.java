package at.knowcenter.recommender.solrpowered.engine.strategy.social.cf.interests.puresocial;
...
public class InterestsJaccardBasedRecPS implements RecommendStrategy {
	...
	public RecommendResponse recommend(RecommendQuery query, Integer maxReuslts){
		...
		try {
			...
			if (contentFilter == null || contentFilter.getCustomer() == null || contentFilter.getCustomer().getInterests() == null) {
			...
			List<String> neighbours = DataFetcher.getSocialNeighbourUsers(query.getUser());
			...
			for (Customer customer : resLists) {				
				Set<String> intersectedInterests = new HashSet<String>(contentFilter.getCustomer().getInterests());
				if (customer.getInterests() == null)
					continue;
				intersectedInterests.retainAll(customer.getInterests());
				
				Set<String> unionInterests = new HashSet<String>(contentFilter.getCustomer().getInterests());
				unionInterests.addAll(customer.getInterests());
				
				customerScoringMap.put(customer.getId(), intersectedInterests.size() / (double)unionInterests.size());
			}
	...
	public StrategyType getStrategyType() {
		return StrategyType.CF_Soc_Interests_Jaccard;
	}
