 ...
 *
 */
public class JaccardBasedRec implements RecommendStrategy {
	...
	public RecommendResponse recommend(RecommendQuery query, Integer maxReuslts){
		...
		try {
			...
			for (String user : otherUsersNeighbors.keySet()) {
				...
				Set<String> unionNeighbors = new HashSet<String>(otherUsersNeighbors.get(user));

				intersectionNeighbors.retainAll(currentUserNeighbors);
				unionNeighbors.addAll(currentUserNeighbors);
				
				commonNeighborMap.put(user, intersectionNeighbors.size() / (double)  unionNeighbors.size());
			}
	...
	public StrategyType getStrategyType() {
		return StrategyType.CF_Soc_Network_Jaccard;
	}
