...
public class GroupPrefAttachmentBasedRec implements RecommendStrategy {
	...
	public RecommendResponse recommend(RecommendQuery query, Integer maxReuslts){
		...
		try {
			...
			for (Customer customer : resLists) {
				customerScoringMap.put(customer.getId(), (double)
						contentFilter.getCustomer().getCustomergroup().size() * customer.getCustomerGroup().size());
			}
	...
	public StrategyType getStrategyType() {
		return StrategyType.CF_Soc_Group_PrefAttach;
	}
