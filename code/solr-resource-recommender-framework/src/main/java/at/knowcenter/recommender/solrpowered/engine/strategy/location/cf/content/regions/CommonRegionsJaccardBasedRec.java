...
public class CommonRegionsJaccardBasedRec implements RecommendStrategy{
	...
	public RecommendResponse recommend(RecommendQuery query, Integer maxReuslts) {
		...
		try {
			...
			for (String commonUser : userRegionsMapping.keySet()) {
				Set<Long> intersection = new HashSet<Long>(userRegionsMapping.get(commonUser));
				Set<Long> union = new HashSet<Long>(userRegionsMapping.get(commonUser));
				
				intersection.retainAll(regions);
			...
			Thread t = new Thread() {
				@Override public void run() {
					UserSimilarityTracker.getInstance().writeToFile("loc_content_monitoredreg_jacc", user, commonNeighborMap);
				}
			};
			t.start();
