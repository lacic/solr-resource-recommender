package at.knowcenter.recommender.solrpowered.engine.strategy.social.cf.groups.puresocial;
...
public class GroupJaccardBasedRecPS implements RecommendStrategy {
	...
	public RecommendResponse recommend(RecommendQuery query, Integer maxReuslts){
		...
		try {
			...
			if (contentFilter == null || contentFilter.getCustomer() == null || contentFilter.getCustomer().getCustomergroup() == null) {
			...
			for (Customer customer : resLists) {
				Set<String> intersectedGroups = new HashSet<String>(contentFilter.getCustomer().getCustomergroup());
				if (customer.getCustomerGroup() == null)
					continue;
				intersectedGroups.retainAll(customer.getCustomerGroup());
				
				Set<String> unionGroups = new HashSet<String>(contentFilter.getCustomer().getCustomergroup());
				unionGroups.addAll(customer.getCustomerGroup());
				
				customerScoringMap.put(customer.getId(), intersectedGroups.size() / (double)unionGroups.size());
			}
	...
	public StrategyType getStrategyType() {
		return StrategyType.CF_Soc_Group_Jaccard;
	}
