...
public class C_Description_Name_Tags_Job extends C_Strategy_Job implements Callable<List<MetricsExporter>>{

	private List<String> users;

	public C_Description_Name_Tags_Job(List<String> users) {
		this.users = users;
	}
	...
	public List<MetricsExporter> evaluate() {
		...
		List<MetricsExporter> metricsCalcs = initMetricCalcs("7_" + StrategyType.CN_WeightDescriptionNameTags.name());
		
		System.out.println("Evaluation over " + users.size() + " users");
		
		evaluate_C_Weighted(cf, metricsCalcs, StrategyType.CN_WeightDescriptionNameTags);
