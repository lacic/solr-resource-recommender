...
public class GroupNeighborOverlapBasedRec implements RecommendStrategy {
	...
	public RecommendResponse recommend(RecommendQuery query, Integer maxReuslts){
		...
		try {
			...
			for (Customer customer : resLists) {
				...
				List<String> sumGroups = new ArrayList<String>(contentFilter.getCustomer().getCustomergroup());
				sumGroups.addAll(customer.getCustomerGroup());
				
				customerScoringMap.put(customer.getId(), intersectedGroups.size() / (double)sumGroups.size());
			}
	...
	public StrategyType getStrategyType() {
		return StrategyType.CF_Soc_Group_Overlap;
	}
