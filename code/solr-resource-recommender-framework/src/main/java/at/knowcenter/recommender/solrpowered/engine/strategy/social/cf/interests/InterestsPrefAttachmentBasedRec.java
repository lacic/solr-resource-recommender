...
public class InterestsPrefAttachmentBasedRec implements RecommendStrategy {
	...
	public RecommendResponse recommend(RecommendQuery query, Integer maxReuslts){
		...
		try {
			...
			for (Customer customer : resLists) {
				customerScoringMap.put(customer.getId(), 
						contentFilter.getCustomer().getInterests().size() / (double)customer.getInterests().size());
			}
	...
	public StrategyType getStrategyType() {
		return StrategyType.CF_Soc_Interests_PrefAttach;
	}
