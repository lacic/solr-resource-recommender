...
public class UB_Interests_Job extends AbstractStrategyJob implements Callable<List<MetricsExporter>>{
	...
	public List<MetricsExporter> call() throws Exception {
		...
		List<MetricsExporter> metricsCalcs = initMetricCalcs("10_" + StrategyType.UB_WithOutMLTInterests.name());
		
		System.out.println("Evaluation over " + users.size() + " users");
		
		evaluate(cf, metricsCalcs, StrategyType.UB_WithOutMLTInterests);
