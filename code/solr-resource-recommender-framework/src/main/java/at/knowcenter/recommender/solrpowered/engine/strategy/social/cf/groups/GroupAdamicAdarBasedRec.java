...
public class GroupAdamicAdarBasedRec implements RecommendStrategy {
	...
	public RecommendResponse recommend(RecommendQuery query, Integer maxReuslts){
		...
		try {
			...
			Map<String, Integer> groupDegrees = new HashMap<String, Integer>();
			...
			if (contentFilter.getCustomer() != null && contentFilter.getCustomer().getCustomergroup() != null) {
				for (String customerGroup : contentFilter.getCustomer().getCustomergroup()) {
					...
					groupDegrees.put(customerGroup, 1);
				}
			...
			for (Customer customer : resLists) {
				for (String group : customer.getCustomerGroup()) {
					Integer degree = groupDegrees.get(group);
					if (degree == null) {
						degree = 1;
					} else {
						degree++;
					}
					groupDegrees.put(group, degree);
				}
			}
			...
			for (Customer customer : resLists) {
				...
				Double aaSum = 0.0;
				for (String group : intersectedGroups) {
					aaSum += 1.0 / Math.log(groupDegrees.get(group));
				}
	...
	public StrategyType getStrategyType() {
		return StrategyType.CF_Soc_Group_AdemicAdar;
	}
