...
public class InterestsAdamicAdarBasedRec implements RecommendStrategy {
	...
	public RecommendResponse recommend(RecommendQuery query, Integer maxReuslts){
		...
		try {
			...
			Map<String, Integer> interestDegrees = new HashMap<String, Integer>();
			...
			if (contentFilter.getCustomer() != null && contentFilter.getCustomer().getInterests() != null) {
				for (String interest : contentFilter.getCustomer().getInterests()) {
					if (!interest.contains("\\")) {
						...
						interestDegrees.put(interest, 1);
					}
				}
			...
			for (Customer customer : resLists) {
				for (String interest : customer.getInterests()) {
					Integer degree = interestDegrees.get(interest);
					if (degree == null) {
						degree = 1;
					} else {
						degree++;
					}
					interestDegrees.put(interest, degree);
				}
			}
			...
			for (Customer customer : resLists) {				
				...
				Double aaSum = 0.0;
				for (String group : intersectedInterests) {
					aaSum += 1.0 / Math.log(interestDegrees.get(group));
				}
				
				
				customerScoringMap.put(customer.getId(), aaSum);
			}
	...
	public StrategyType getStrategyType() {
		return StrategyType.CF_Soc_Interests_AdemicAdar;
	}
