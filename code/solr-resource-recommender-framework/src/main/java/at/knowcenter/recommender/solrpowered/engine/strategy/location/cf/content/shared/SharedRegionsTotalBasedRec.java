package at.knowcenter.recommender.solrpowered.engine.strategy.location.cf.content.shared;
...
public class SharedRegionsTotalBasedRec implements RecommendStrategy{
...
String q = "id:" + user;
			...
			StringBuilder queryBuilder = new StringBuilder("shared_region_id:(");
			
			for (String sharedRegion : sharedRegions){
				queryBuilder.append(sharedRegion + " OR ");
			}
			...
			for (SharedLocation sharedLocation : sharedLocations){
				...
				Set<String> union = new HashSet<String>(sharedLocation.getSharedRegions());
				
				union.addAll(sharedRegions);
			...
			Thread t = new Thread() {
				@Override public void run() {
					UserSimilarityTracker.getInstance().writeToFile("loc_content_shared_tot", user, commonNeighborMap);
				}
			};
			t.start();
	...
	public StrategyType getStrategyType() {
		return StrategyType.CF_Loc_Shared_Regions_Total;
	}
