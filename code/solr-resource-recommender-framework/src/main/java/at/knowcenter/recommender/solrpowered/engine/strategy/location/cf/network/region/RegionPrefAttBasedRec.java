...
public class RegionPrefAttBasedRec implements RecommendStrategy{
	...
	public RecommendResponse recommend(RecommendQuery query, Integer maxReuslts) {
		...
		try {
			...
			solrParams.set("sort", "region_neighborhood_count desc");
			...
			for (PositionNetwork otherPosition : otherPositions) {
				commonNeighborMap.put(otherPosition.getUserId(), 
						locationNeighbors.size() * (double)otherPosition.getRegionNeighborsCount());
			}
	...
	public StrategyType getStrategyType() {
		return StrategyType.CF_Region_Network_All_PrefAtt;
	}
