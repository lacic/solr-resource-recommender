package at.knowcenter.recommender.solrpowered.evaluation.concurent.jobs.social.cf;
...
public class CF_Social_Likes_Job extends RecommenderEvaluator implements Callable<List<MetricsExporter>>{

	private List<String> users;

	public CF_Social_Likes_Job(List<String> users, String jobDescription) {
		this.users = users;
	...
	public List<MetricsExporter> evaluate() {
		...
		for (int n = 1; n <= resultSize; n++) {
			...
			mCalc.setAlgorithmName("9_CF_Social_Likes");
	...
	protected void evaluate_CF_Social(List<String> users, ContentFilter cf, List<MetricsExporter> metricsCalcs) {
		...
		for (String userID : users) {
			...
			List<String> recommendations = getRecommendations(userID, null, resultSize, cf, recommendStrategies.get(StrategyType.CF_Social_Likes));
			...
			System.out.println("CF_Social_Likes_+" + jobDescription +": Evaluation progress: " + ((currentEvaluatingUser) / (double)userSize) * 100 + " % done");
		}
	}
