...
//		this.simulatedUpdatesCount = simulatedUpdateCount;
	...
	public List<MetricsExporter> evaluate() {
		...
		for (int n = 1; n <= resultSize; n++) {
			...
			mCalc.setAlgorithmName("3_CF-CFcat-C_");
			metricsCalcs.add(mCalc);
		}
		
		System.out.println("Evaluation over " + users.size() + " users");
		
		evaluate_CF_CFcat_C(users, cf, metricsCalcs);
		return metricsCalcs;
	}
	
	protected void evaluate_CF_CFcat_C(List<String> users, ContentFilter cf, List<MetricsExporter> metricsCalcs) {
